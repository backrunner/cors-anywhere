...
function showUsage(help_file, headers, response, retry = false) {
  const isHtml = /\.html$/.test(help_file);
  headers['Content-Type'] = isHtml ? 'text/html' : 'text/plain';
  if (help_text[help_file] || retry) {
    response.writeHead(200, headers);
    response.end(help_text[help_file] || 'Nothing here.');
  } else {
    require('fs').readFile(help_file, 'utf8', function(err, data) {
      if (err) {
        ...
        showUsage(help_file, headers, response, true); // Recursive call, but since data is a string, the recursion will end
      }
    });
  }
}
...
function withCORS(headers, request) {
  ...
  if (request.headers['Access-Control-Request-Method']) {
    headers['Access-Control-Request-Method'] = request.headers['Access-Control-Request-Method'];
    delete request.headers['Access-Control-Request-Method'];
  }
  if (request.headers['Access-Control-Request-Headers']) {
    headers['Access-Control-Request-Headers'] = request.headers['Access-Control-Request-Headers'];
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  const { corsAnywhereRequestState: requestState } = req;
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    for (const toRemove of corsAnywhere.removeHeaders) {
      for (const pair of req.headers.entries()) {
        if (toRemove instanceof RegExp && toRemove.test(pair[0])) {
          delete req.headers[pair[0]];
          continue;
        }
        if (typeof toRemove === 'string' && toRemove === pair[0]) {
          delete req.headers[pair[0]];
          continue;
        }
      }
